...
+ **서브쿼리?**
  + 하나의 SQL 문에 포함되어 있는 또 다른 SQL 문을 말함
  + SELECT 절에 사용하는 서브쿼리 스칼라 서브쿼리라고도 함.
  + 스칼라 서브쿼리는 한 행, 한 컬럼만을 반환하는 서브쿼리를 말함.
  + FROM 절에 사용하는 서브쿼리
  + 인라인 뷰 라고 함
  + 기본적으로 FROM 절에는 테이블 명이 오도록 되어있습니다. 그런데 서브쿼리가 FROM 절에 사용되면 동적으로 생성된 테이블인 것처럼 사용할 수 있음
